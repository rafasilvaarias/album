<script lang="ts">
  ...
  function scheduleQueue(
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const attack = (resolveRange(sample.attack, sample.attackMin, sample.attackMax) ?? 0.001 / beatDuration) * beatDuration;
      let decay = (resolveRange(sample.decay, sample.decayMin, sample.decayMax) ?? 0.001 / beatDuration) * beatDuration;
      decay = Math.min(decay, dur - attack - 0.001);
      gainNode.gain.cancelScheduledValues(startTime);
      gainNode.gain.setValueAtTime(0, startTime);
      gainNode.gain.exponentialRampToValueAtTime(gain, startTime + attack);
      gainNode.gain.setValueAtTime(gain, startTime + dur - decay - 0.001);
      gainNode.gain.exponentialRampToValueAtTime(0, startTime + dur - 0.001);
    }
    return [];
  }
